...
public class LayananTugas {
    ...
    public LayananTugas(FirebaseStorage firebaseStorage) {
        ...
        this.idBerikutnya = 1;
        // Data akan dimuat nanti lewat refreshData() di latar belakang
    }

    public List<Tugas> ambilDataDariCloud() {
        List<Tugas> muat = firebaseStorage.muatTugas();
        if (muat != null) {
            this.daftarTugas.clear();
            this.daftarTugas.addAll(muat);
            this.idBerikutnya = daftarTugas.isEmpty() ? 1
                    : daftarTugas.stream().map(Tugas::getId).max(Integer::compareTo).orElse(0) + 1;
        }
        return new ArrayList<>(this.daftarTugas);
    }
